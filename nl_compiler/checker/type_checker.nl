...
def check_types_imported(type : @tct::meta_type, ref modules : @tc_types::modules_t, ref errors : @tc_types::errors_t) : ptd::void(){
...
def type_checker::check(modules : ptd::hash(@nast::module_t), lib_modules : ptd::hash(@nast::module_t)) : @tc_types::return_t {
	return type_checker::check_modules(modules, lib_modules);
}

def type_checker::check_modules(modules : ptd::hash(@nast::module_t), lib_modules : ptd::hash(@nast::module_t)) : @tc_types::return_t {
...
def check_module(module : @nast::module_t, ref def_fun : @tc_types::defs_funs_t, ref errors : @
	tc_types::errors_t, ref deref : @tc_types::deref_types) : ptd::void() {
	var modules : @tc_types::modules_t = {env=>{deref=>deref, current_module => module->name, breaks => {vars=>{}, is=>false}, ret_type=>tct::tct_empty()}, imports => {}, funs => def_fun};
...
def join_vars(ref vars : @tc_types::vars_t, vars_op : @tc_types::vars_t, ref modules : @tc_types::modules_t, ref errors : @tc_types::errors_t) : ptd::void() {
	...
	forh var var_name, var var_ (vars) {
		match (var_->overwrited) case :yes {
			...
			hash::set_value(ref vars, var_name, {
					overwrited => :yes,
					type => ptd_system::cross_type(t1, t2, ref modules, ref errors)
				});
		} case :no {
		}
	}
...
def set_end_function(ref vars : @tc_types::vars_t) : ptd::void() {
	hash::set_value(ref vars, '__END', {overwrited => :yes,type => tct::tct_empty()});
}

def check_cmd(cmd : @nast::cmd_t, ref modules : @tc_types::modules_t, ref b_vars : @tc_types::vars_t, ref 
	errors : @tc_types::errors_t) : ptd::hash(@tc_types::var_t) {
	...
	match (cmd->cmd) case :if(var as_if) {
		...
		add_error(ref errors, 'if argument should be sim or boolean instead of ' . get_print_tct_type_name(if_cond_type->type))
			unless
			ptd_system::is_condition_type(if_cond_type, ref modules, ref errors);
		...
		fora var elsif_s (as_if->elsif) {
			...
			add_error(ref errors, 'elsif condition should be sim or boolean instead of '.get_print_tct_type_name(elsif_cond->type))
				unless
				ptd_system::is_condition_type(elsif_cond, ref modules, ref errors);
		...
		if(!(as_for->cond is :nop)){
			...
			add_error(ref errors, 'for condition should be sim or boolean instead of '.get_print_tct_type_name(for_cond->type))
				unless
				ptd_system::is_condition_type(for_cond, ref modules, ref errors);
		...
		add_error(ref errors, 'if argument should be sim or boolean type instead of ' . get_print_tct_type_name(if_cond_type->type))
			unless
			ptd_system::is_condition_type(if_cond_type, ref modules, ref errors);
		var vars_op : @tc_types::vars_t = vars;
		...
		add_error(ref errors, 'unless argument should be sim or boolean type instead of ' . get_print_tct_type_name(unless_cond_type->type))
			unless
			ptd_system::is_condition_type(unless_cond_type, ref modules, ref errors);
		var vars_op : @tc_types::vars_t = vars;
		...
		if(ov::is(modules->env->ret_type, 'tct_void') && !ov::is(ret_type->type, 'tct_empty')){
			add_error(ref errors, 'can''t return value in void function');
		}elsif(!ov::is(modules->env->ret_type, 'tct_void') && !ov::is(modules->env->ret_type, 'tct_im') && ov::is(ret_type->type, 'tct_empty')){
			add_error(ref errors, 'must be returned value in non void function');
		}elsif(!ov::is(modules->env->ret_type, 'tct_void') && !ov::is(ret_type->type, 'tct_empty')){
		...
		ret = check_try_ensure(as_try, ref modules, ref vars, ref errors);
	} case :ensure(var as_ensure) {
		ret = check_try_ensure(as_ensure, ref modules, ref vars, ref errors);
	} case :nop {
	}
	...
	if(hash::has_key(vars, '__END')) {
		hash::set_value(ref b_vars, '__END', {overwrited => :yes,type => tct::tct_empty()});
	}
	return ret;
}
def break_continue_block(cmd : @nast::cmd_t, ref modules : @tc_types::modules_t, 
	ref vars : @tc_types::vars_t, ref errors : @tc_types::errors_t) : ptd::void() {
...
def check_try_ensure(try_ensure : @nast::try_ensure_t, ref modules : @tc_types::modules_t, ref vars : @
	tc_types::vars_t, ref errors : @tc_types::errors_t) : ptd::hash(@tc_types::var_t) {
	var ret : ptd::hash(@tc_types::var_t) = {};
	match (try_ensure) case :decl(var decl) {
		hash::set_value(ref ret, decl->name, check_var_decl_try(decl, true, ref modules, ref vars, ref errors));
	} case :lval(var lval) {
		var type = check_val(lval->right, ref modules, ref vars, ref errors);
		var err_left_len = array::len(errors->errors);
		var left_type : @tc_types::type = get_type_left_site_equation(lval->left, ref modules, ref vars, ref errors);
		err_left_len = array::len(errors->errors) - err_left_len;
		return ret if (err_left_len > 0);
		type->type = ptd_system::get_try_ensure_ok_type(type, ref modules, ref errors);
		...
		ptd_system::get_try_ensure_ok_type(type, ref modules, ref errors);
	}
	return ret;
}

def check_forh(as_forh : @nast::forh_t, ref modules : @tc_types::modules_t, ref vars : @tc_types::vars_t, ref 
	errors : @tc_types::errors_t) : ptd::void() {
	var hash_type : @tc_types::type = ptd_system::can_delete(check_val(as_forh->hash, ref modules, ref vars, ref errors), ref modules, ref errors);
	if (ptd_system::is_accepted(hash_type, tct::tct_hash(tct::tct_im()), ref modules, ref errors)) {
	} elsif (ptd_system::is_accepted(hash_type, tct::tct_rec({}), ref modules, ref errors)) {
		add_error(ref errors, 'forh argument should be a hash not rec') if is_known(hash_type);
	} else {
		add_error(ref errors, 'forh argument should be a hash type instead of '.get_print_tct_type_name(hash_type->type));
	}

	hash_type->type = ov::is(hash_type->type, 'tct_hash')
			? hash_type->type as :tct_hash
			: tct::tct_im();
	var vars_op : @tc_types::vars_t = vars;
	add_var_decl_with_type_and_check(as_forh->key, {type=>tct::tct_sim(), src=>:speculation}, ref vars_op, ref errors);
...
def check_fora(as_fora : @nast::fora_t, ref modules : @tc_types::modules_t, ref vars : @tc_types::vars_t, ref 
	errors : @tc_types::errors_t) : ptd::void() {
	...
	add_error(ref errors, 'fora argument should be an array instead of '.get_print_tct_type_name(fora_arr_type->type))
		unless
		ptd_system::is_accepted(fora_arr_type, tct::tct_arr(tct::tct_im()), ref modules, ref errors);
	fora_arr_type->type = ov::is(fora_arr_type->type, 'tct_arr')
			? fora_arr_type->type as :tct_arr
			: tct::tct_im();
	var vars_op : @tc_types::vars_t = vars;
...
def check_while(as_while : @nast::while_t, ref modules : @tc_types::modules_t, ref vars : @tc_types::vars_t, 
	ref errors : @tc_types::errors_t) : ptd::void() {
	var cond_type : @tc_types::type = check_val(as_while->cond, ref modules, ref vars, ref errors);
	add_error(ref errors, 'while argument should be sim or boolean type insteand of ' . get_print_tct_type_name(cond_type->type))
		unless
		ptd_system::is_condition_type(cond_type, ref modules, ref errors);
	var vars_op : @tc_types::vars_t = vars;
	break_continue_block(as_while->cmd, ref modules, ref vars_op, ref errors);
	join_vars(ref vars, vars_op, ref modules, ref errors);
}

def check_rep(as_rep : @nast::rep_t, ref modules : @tc_types::modules_t, ref vars : @tc_types::vars_t, ref 
	errors : @tc_types::errors_t) : ptd::void() {
	...
	add_error(ref errors, 'rep argument should be a number instead of '.get_print_tct_type_name(count_type->type))
		unless
		ptd_system::is_accepted(count_type, tct::tct_sim(), ref modules, ref errors);
	var vars_op : @tc_types::vars_t = vars;
	add_var_decl_with_type_and_check(as_rep->iter, {type=>tct::tct_sim(), src=>:speculation}, ref vars_op, ref errors);
	break_continue_block(as_rep->cmd, ref modules, ref vars_op, ref errors);
	join_vars(ref vars, vars_op, ref modules, ref errors);
}

def check_match(as_match : @nast::match_t, ref modules : @tc_types::modules_t, ref vars : @tc_types::vars_t, 
	ref errors : @tc_types::errors_t) : ptd::void() {
	...
	if(!ptd_system::is_accepted(val_type, tct::tct_var({}), ref modules, ref errors)){
...
def check_val(val : @nast::value_t, ref modules : @tc_types::modules_t, ref vars : @tc_types::vars_t, ref 
	errors : @tc_types::errors_t) : @tc_types::type {
	var ret = {type=>tct::tct_im(), src=>:speculation};
	match (val) case :ternary_op(var ternary_op) {
		var cond_type = check_val(ternary_op->fst, ref modules, ref vars, ref errors);
		add_error(ref errors, 'ternary op first argument should be sim or boolean type instead of ' . get_print_tct_type_name(cond_type->type))
			unless
			ptd_system::is_condition_type(cond_type, ref modules, ref errors);
		...
		ret->type = tct::tct_sim();
	} case :nop {
		ret->type = tct::tct_empty();
		...
		fora var dec (arr_decl){
			...
			is_none = true if !is_known(t);
		}
		var rt = types[0]->type;
		ptd_system::check_assignment(tct::tct_im(), types[0], ref modules, ref errors) if (is_none);
		for(var i = 1;i<array::len(types);++i) {
			rt = ptd_system::cross_type(rt, types[i]->type, ref modules, ref errors);
			ptd_system::check_assignment(tct::tct_im(), types[i], ref modules, ref errors) if (is_none);
		}
		ret->src = :known unless is_none;
		ret->type = tct::tct_arr(rt);
	} case :hash_decl(var hash_decl) {
		var rt = {};
		var is_none = false;
		fora var el (hash_decl) {
			var label = ov::as(el->key, 'hash_key');
			var ty = check_val(el->val, ref modules, ref vars, ref errors);
			hash::set_value(ref rt, label, ty);
			is_none = true if !is_known(ty);
		}
		var retur = {};
		forh var key, var ty (rt) {
			ptd_system::check_assignment(tct::tct_im(), ty, ref modules, ref errors) if (is_none);
			hash::set_value(ref retur, key, ty->type);
		}
		ret->src = :knownhash if !is_none;
		ret->type = tct::tct_rec(retur);
		...
		var left_type2 : @tc_types::type = ptd_system::can_delete(check_val(var_op->left, ref modules, ref vars, ref errors), 
				ref modules, ref errors);
		var ret_type : @tc_types::type; 
		match(var_op->op) case :ov_is{
			ret_type = {type=>tct::tct_bool(), src=>:speculation};
		} case :ov_as{
			ret_type = {type=>tct::tct_im(), src=>left_type2->src};
		}
		if(ptd_system::is_accepted(left_type2, tct::tct_var({}), ref modules, ref errors)){
			return ret_type unless ov::is(left_type2->type, 'tct_var');
			var variants = left_type2->type as :tct_var;
			if(!hash::has_key(variants, var_op->case)){
				add_error(ref errors, 'case '.var_op->case.' don''t exists is: ' . get_print_tct_type_name(left_type2->type));
				return ret_type;
			}
			match(var_op->op) case :ov_is{
			} case :ov_as{
				...
				match(t) case :with_param(var sub_type){
					...
					add_error(ref errors, 'case '.var_op->case.' don''t have value: ' . get_print_tct_type_name(left_type2->type));
				}
			}
			return ret_type;
		} else{
		...
		if (unary_op->op eq '!') {
			if(!ptd_system::is_condition_type(type, ref modules, ref errors)){
				add_error(ref errors, 'incorrect type of argument operator ''!'' : ' .get_print_tct_type_name(type->type));
			}
			ret->type = tct::tct_bool();
			return ret;
		}elsif(unary_op->op eq '@'){
			if(!ptd_system::is_accepted(type, tct::tct_sim(), ref modules, ref errors)){
				add_error(ref errors, 'incorrect type of argument operator '''.unary_op->op.''' : ' .get_print_tct_type_name(type->type));
			}
			ret->type = tct::tct_var({ref => tct::tct_sim()});
			return ret;
		}else{
			if(!ptd_system::is_accepted(type, tct::tct_sim(), ref modules, ref errors)){
				add_error(ref errors, 'incorrect type of argument operator '''.unary_op->op.''' : ' .get_print_tct_type_name(type->type));
			...
			if(is_spec){
				...
				if(ov::is(fun_def_arg->mod, 'ref')){
					...
					if (err_len == array::len(errors->errors)){
						var var_name = ov::as(var_tab[0], 'var');
						...
						set_type_to_lval(fun_val_arg->val, args_values_types[i], args_values_types[i], ref modules, ref vars, ref errors);
					}
				}
				check_info = ptd_system::check_assignment(fun_def_arg->type, args_values_types[i], ref modules, ref errors);
			}
			if(ov::is(check_info, 'err')){
				add_error(ref errors, 'In function call: '. get_function_name(fun_val->module, fun_val->name).' argument no.' . (i + 1) . ' ''' .
					fun_def_arg->name . ''' has invalid type: '. get_print_check_info(check_info));
			}
		}
		ret = check_special_function({type => fun_def->ret_type, src => :known}, fun_val, args_values_types, ref modules, ref vars, ref errors);
	} case :string(var str) {
		ret->type = tct::tct_sim();
	} case :post_inc(var inc) {
		var vtype = check_val(inc, ref modules, ref vars, ref errors);
		if(!ptd_system::is_accepted(vtype, tct::tct_sim(), ref modules, ref errors)){
			add_error(ref errors, 'wrong type in post increment : '.get_print_tct_type_name(vtype->type));
		}
		ret->type = tct::tct_sim();
	} case :post_dec(var dec) {
		var vtype = check_val(dec, ref modules, ref vars, ref errors);
		if(!ptd_system::is_accepted(vtype, tct::tct_sim(), ref modules, ref errors)){
			add_error(ref errors, 'wrong type in post decrement : '.get_print_tct_type_name(vtype->type));
		}
		ret->type = tct::tct_sim();
	}
	return ret;
...
def get_special_functions() : @tc_types::special_functions {
	...
	hash::set_value(ref f,'ptd::ensure',{r=>tct::tct_im(), a=>[
		{mod=>:none, type=>tct::tct_im(), name=>''},
		{mod=>:none, type=>tct::tct_im(), name=>''}]});
	hash::set_value(ref f,'ptd::ensure_only_static_do_not_touch_without_permission',{r=>tct::tct_im(), a=>[
		{mod=>:none, type=>tct::tct_im(), name=>''},
		{mod=>:none, type=>tct::tct_im(), name=>''}]});
	
	hash::set_value(ref f,'array::push',{r=>tct::tct_void(), a=>[
		{mod=>:ref,  type=>tct::tct_arr(tct::tct_im()), name=>''},
		{mod=>:none, type=>tct::tct_im(), name=>''}]});
	hash::set_value(ref f,'array::subarray',{r=>tct::tct_im(), a=>[
			{mod=>:none,  type=>tct::tct_arr(tct::tct_im()), name=>''},
			{mod=>:none, type=>tct::tct_sim(), name=>''},
			{mod=>:none, type=>tct::tct_sim(), name=>''},
		]});
	hash::set_value(ref f,'array::join',{r=>tct::tct_sim(), a=>[
			{mod=>:none,  type=>tct::tct_sim(), name=>''},
			{mod=>:none,  type=>tct::tct_arr(tct::tct_sim()), name=>''},
		]});
	hash::set_value(ref f,'array::append',{r=>tct::tct_void(), a=>[
			{mod=>:ref,  type=>tct::tct_arr(tct::tct_im()), name=>''},
			{mod=>:none,  type=>tct::tct_arr(tct::tct_im()), name=>''},
		]});
	hash::set_value(ref f,'array::len',{r=>tct::tct_sim(), a=>[
			{mod=>:none,  type=>tct::tct_arr(tct::tct_im()), name=>''},
		]});
	
	hash::set_value(ref f,'hash::set_value',{r=>tct::tct_void(), a=>[
			{mod=>:ref,  type=>tct::tct_hash(tct::tct_im()), name=>''},
			{mod=>:none, type=>tct::tct_sim(), name=>''},
			{mod=>:none, type=>tct::tct_im(), name=>''}
		]});
	hash::set_value(ref f,'hash::get_value',{r=>tct::tct_im(), a=>[
		{mod=>:none, type=>tct::tct_hash(tct::tct_im()), name=>''},
		{mod=>:none, type=>tct::tct_sim(), name=>''}]});
	hash::set_value(ref f,'hash::has_key',{r=>tct::tct_bool(), a=>[
		{mod=>:none, type=>tct::tct_hash(tct::tct_im()), name=>''},
		{mod=>:none, type=>tct::tct_sim(), name=>''}]});
	hash::set_value(ref f,'hash::delete',{r=>tct::tct_void(), a=>[
		{mod=>:ref, type=>tct::tct_hash(tct::tct_im()), name=>''},
		{mod=>:none, type=>tct::tct_sim(), name=>''}]});
	hash::set_value(ref f,'hash::size',{r=>tct::tct_sim(), a=>[
		{mod=>:none, type=>tct::tct_hash(tct::tct_im()), name=>''}]});
	hash::set_value(ref f,'hash::values',{r=>tct::tct_arr(tct::tct_im()), a=>[
		{mod=>:none, type=>tct::tct_hash(tct::tct_im()), name=>''}]});

	hash::set_value(ref f,'ov::is',{r=>tct::tct_bool(), a=>[
			{mod=>:none,  type=>tct::tct_var({}), name=>''},
			{mod=>:none,  type=>tct::tct_sim(), name=>''},
		]});
	hash::set_value(ref f,'ov::as',{r=>tct::tct_im(), a=>[
			{mod=>:none,  type=>tct::tct_var({}), name=>''},
			{mod=>:none,  type=>tct::tct_sim(), name=>''},
		]});
	
	hash::set_value(ref f,'dfile::ssave',{r=>tct::tct_sim(), a=>[
			{mod=>:none,  type=>tct::tct_im(), name=>''},
		]});
		
	hash::set_value(ref f,'string::lf',{r=>tct::tct_sim(), a=>[]});
	hash::set_value(ref f,'string::length',{r=>tct::tct_sim(), a=>[
			{mod=>:none,  type=>tct::tct_sim(), name=>''},
		]});
	hash::set_value(ref f,'string::substr',{r=>tct::tct_sim(), a=>[
			{mod=>:none,  type=>tct::tct_sim(), name=>''},
			{mod=>:none,  type=>tct::tct_sim(), name=>''},
			{mod=>:none,  type=>tct::tct_sim(), name=>''},
		]});
	hash::set_value(ref f,'string::replace',{r=>tct::tct_sim(), a=>[
			{mod=>:none,  type=>tct::tct_sim(), name=>''},
			{mod=>:none,  type=>tct::tct_sim(), name=>''},
			{mod=>:none,  type=>tct::tct_sim(), name=>''},
		]});
	hash::set_value(ref f,'string::chr',{r=>tct::tct_sim(), a=>[
			{mod=>:none,  type=>tct::tct_sim(), name=>''},
		]});
	hash::set_value(ref f,'string::ord',{r=>tct::tct_sim(), a=>[
			{mod=>:none,  type=>tct::tct_sim(), name=>''},
		]});
	hash::set_value(ref f,'string::split',{r=>tct::tct_arr(tct::tct_sim()), a=>[
			{mod=>:none,  type=>tct::tct_sim(), name=>''},
			{mod=>:none,  type=>tct::tct_sim(), name=>''},
		]});
	hash::set_value(ref f, 'c_std_lib::fast_substr', {r=>tct::tct_sim(), a=>[
			{mod=>:none,  type=>tct::tct_im(), name=>''},
			{mod=>:none,  type=>tct::tct_im(), name=>''},
			{mod=>:none,  type=>tct::tct_im(), name=>''},
		]});
	hash::set_value(ref f,'c_singleton::sigleton_do_not_use_without_approval',{r=>tct::tct_im(), a=>[
			{mod=>:none,  type=>tct::tct_im(), name=>''},
		]});
...
def check_special_function(ret_type : @tc_types::type, fun_val : @nast::fun_val_t, fun_val_type : ptd::arr(@tc_types::type),
		ref modules : @tc_types::modules_t, ref vars : @tc_types::vars_t, ref errors : @tc_types::errors_t) : @tc_types::type {
	...
	if (name eq 'array::push') {
		fun_val_type[1]->type = tct::tct_arr(fun_val_type[1]->type);
		set_type_to_lval_spec(fun_val->args[0]->val, fun_val_type[0], fun_val_type[1], 
			tct::tct_arr(tct::tct_empty()), ref modules, ref vars, ref errors);
	}
	...
	if (name eq 'hash::set_value') {
		fun_val_type[2]->type = tct::tct_hash(fun_val_type[2]->type);
		set_type_to_lval_spec(fun_val->args[0]->val, fun_val_type[0], fun_val_type[2], 
			tct::tct_rec({}), ref modules, ref vars, ref errors);
	}
	if (name eq 'hash::delete') {
		var type = ptd_system::can_delete(fun_val_type[0], ref modules, ref errors);
		if(type->type is :tct_rec){
			var cross = type;
			cross->type = ptd_system::cross_type(tct::tct_hash(tct::tct_empty()), type->type, ref modules, ref errors);
			set_type_to_lval_spec(fun_val->args[0]->val, type, cross, tct::tct_rec({}), ref modules, ref vars, ref errors);
		}
	}
	...
	if (name eq 'hash::values') {
		var type = ptd_system::can_delete(fun_val_type[0], ref modules, ref errors);
		ret_type->src = type->src;
		ret_type->type = tct::tct_arr(type->type as :tct_hash) if type->type is :tct_hash;
	}
	if (name eq 'ov::as') {
...
def rec_get_var_from_lval(lval : @nast::value_t, ref errors : @tc_types::errors_t) : @tc_types::lval_path {
	...
	if (ov::is(lval, 'var')) {
		var name = lval as :var;
		array::push(ref a, :var(name));
	} elsif (ov::is(lval, 'bin_op')) {
		var bin_op = lval as :bin_op;
		if (bin_op->op eq '->') {
			a = rec_get_var_from_lval(bin_op->left, ref errors);
			array::push(ref a, :rec(bin_op->right as :hash_key));
...
def mk_new_type_lval(var_tab : @tc_types::lval_path, rtype : @tc_types::type, ltype : @tc_types::type,
		empty_type : @tct::meta_type, ref modules : @tc_types::modules_t, ref errors : @tc_types::errors_t) : @tct::meta_type {
	...
	if (array::len(var_tab) == 0) {
		...
		match (var_tab[0]) case :var(var name) {
			...
			return tct::tct_im() unless ov::is(ltype->type, 'tct_arr');
			ltype->type = ltype->type as :tct_arr;
			return tct::tct_arr(mk_new_type_lval(var_tab, rtype, ltype, empty_type, ref modules, ref errors));
		} case :rec(var name) {
			return tct::tct_im() unless ov::is(ltype->type, 'tct_rec');
			var tt : ptd::hash(@tct::meta_type) = ltype->type as :tct_rec;
			if (hash::has_key(tt, name)) {
				ltype->type = hash::get_value(tt, name);
				hash::set_value(ref tt, name, mk_new_type_lval(var_tab, rtype, ltype, empty_type, ref modules, ref errors));
			} else {
			...
			return tct::tct_rec(tt);
		}
	}
}
def set_type_to_lval(lval : @nast::value_t, ltype : @tc_types::type, rtype : @tc_types::type, ref 
	modules : @tc_types::modules_t, ref vars : @tc_types::vars_t, ref errors : @tc_types::errors_t) : @tc_types::type {
	return set_type_to_lval_spec(lval, ltype, rtype, ltype->type, ref modules, ref vars, ref errors);
}
def set_type_to_lval_spec(lval : @nast::value_t, ltype : @tc_types::type, rtype : @tc_types::type,
		empty_type : @tct::meta_type, ref modules : @tc_types::modules_t,
		ref vars : @tc_types::vars_t, ref errors : @tc_types::errors_t) : @tc_types::type {
	var err_len = array::len(errors->errors);
	...
	if(!hash::has_key(vars, var_name)){
		add_error(ref errors, 'variable '.var_name.' dont exist');
		return ltype;
	}
	var var_ = hash::get_value(vars, var_name);
	match (var_->overwrited) case :yes {
		var new_type = mk_new_type_lval(var_tab, rtype, {type=>var_->type, src=>:speculation}, empty_type, ref modules, ref errors);
		hash::set_value(ref vars, var_name, {overwrited => :yes, type => new_type});
		return rtype;
	} case :no {
...
def get_type_left_site_equation(left : @nast::value_t, ref modules : @tc_types::modules_t, ref vars : @
	tc_types::vars_t, ref errors : @tc_types::errors_t) : @tc_types::type {
	if (ov::is(left, 'bin_op') && (left as :bin_op)->op eq '->') {
		return get_type_record_key(left as :bin_op, ref modules, ref vars, ref errors);
	} else {
		return check_val(left, ref modules, ref vars, ref errors);
	}
}
def get_type_record_key(bin_op : @nast::bin_op_t, ref modules : @tc_types::modules_t, ref vars : 
		@tc_types::vars_t, ref errors : @tc_types::errors_t) : @tc_types::type {
	...
	if(left_type->type is :tct_rec){
		...
		var field = bin_op->right as :hash_key;
	...
	if(!ptd_system::is_accepted(left_type, tct::tct_rec({}), ref modules, ref errors)){
...
def get_type_from_bin_op_and_check(bin_op : @nast::bin_op_t, ref modules : @tc_types::modules_t, ref vars : @
	...
	if (op eq '=') {
		...
		var err_left_len = array::len(errors->errors);
		var left_type : @tc_types::type = get_type_left_site_equation(bin_op->left, ref modules, ref vars, ref errors);
	...
	if (op eq 'ARRAY_INDEX') {
		if (!ptd_system::is_accepted(left_type2, tct::tct_arr(tct::tct_im()), ref modules, ref errors)) {
			add_error(ref errors, 'array operator ''[]'' can be applied only to array');
			return ret_type;
		}
		if (!ptd_system::is_accepted(right_type, tct::tct_sim(), ref modules, ref errors)) {
				...
				if(hash::has_key(to_types, name)){
					match (from_type) case :no_param{
						if(hash::get_value(to_types, name) is  :with_param){
							return ret . '(ptd::var case: ''' . name . ''' no_param instead of ptd::var case: '''.name.''' with_param)';
						}
					} case :with_param(var f_t){
						if(hash::get_value(to_types, name) is  :no_param){
							return ret . '(ptd::var case: ''' . name . ''' with_param instead of ptd::var case: '''.name.''' no_param)';
						}
					}
				}else{
...
def check_var_decl(var_decl : @nast::variable_declaration_t, ref modules : @tc_types::modules_t, ref vars : @
	...
	return check_var_decl_try(var_decl, false, ref modules, ref vars, ref errors);
}
...
def check_var_decl_try(var_decl : @nast::variable_declaration_t, is_try : @boolean_t::type, ref modules : @
	...
	var var_type : @tct::meta_type = tct::tct_empty();
	...
	match (var_decl->value) case :value(var value) {
		...
		if(ov::is(assign_type->type, 'tct_void')){
			add_error(ref errors, 'cannot use ''void'' type returned from function as value');
			return {overwrited => :yes, type => tct::tct_im()};
		}
		if (is_try) {
			assign_type->type = ptd_system::get_try_ensure_ok_type(assign_type, ref modules, ref errors);
		}
		if(ov::is(var_type, 'tct_empty')){
			ptd_system::check_assignment(tct::tct_im(), assign_type, ref modules, ref errors);
			var_type = assign_type->type;
			return {overwrited => :yes, type => var_type};
		}else{
			var check_info = ptd_system::check_assignment(var_type, assign_type, ref modules, ref errors);
			if(ov::is(check_info, 'err')){
				add_error(ref errors, 'invalid type in variable declaration: '.get_print_check_info(check_info));
			}
			return {overwrited => :no, type => var_type};
		}
	} case :none {
		return {overwrited => :yes, type => var_type} if(ov::is(var_type, 'tct_empty'));
	}
	return {overwrited => :no, type => var_type};
}
...
def add_var_decl_with_type_and_check(var_decl : @nast::variable_declaration_t, type : @tc_types::type,
		ref vars : @tc_types::vars_t, ref errors : @tc_types::errors_t) : ptd::void() {
	add_error(ref errors, 'variable `' . var_decl->name . ''' already exists') if hash::has_key(vars, var_decl->name);
	if(is_known(type)){
...
def check_function_exists(fun_module : ptd::sim(), fun_name : ptd::sim(), ref modules : @tc_types::modules_t, ref 
