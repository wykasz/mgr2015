...
def ptd_system::is_equal(a : @tct::meta_type, b : @tct::meta_type) : @boolean_t::type {
	...
	match (a) case :tct_im {
		...
		forh var name, var type (vars) {
			...
			continue if ov::is(type, 'no_param') && ov::is(to_type, 'no_param');
			return false unless ov::is(type, 'with_param') && ov::is(to_type, 'with_param');
			return false unless ptd_system::is_equal(
				to_type as :with_param,
				type as :with_param
			);
		}
	} case :tct_empty {
...
def ptd_system::is_try_ensure_type(type : @tc_types::type, ref modules : @tc_types::modules_t, ref errors : @
	...
	return ptd_system::is_accepted(type, tct::tct_var({ok=>tct::tct_im(), err=>tct::tct_im()}), ref modules, ref errors);
}
def ptd_system::get_try_ensure_ok_type(type : @tc_types::type, ref modules : @tc_types::modules_t, ref errors : @
	...
	if(ptd_system::is_try_ensure_type(type, ref modules, ref errors)){
		return tct::tct_im() if ov::is(type->type, 'tct_im');
		var t = hash::get_value(type->type as :tct_var, 'ok');
		return tct::tct_empty() if (ov::is(t, 'no_param'));
		return t as :with_param;
	} else {
...
def ptd_system::is_condition_type(from : @tc_types::type, ref modules : @tc_types::modules_t, ref errors : @
	tc_types::errors_t) : @boolean_t::type {
	return ptd_system::is_accepted(from, tct::tct_bool(), ref modules, ref errors);
}

def ptd_system::is_accepted(from : @tc_types::type, as_type : @tct::meta_type, ref modules : @tc_types::modules_t, ref errors : @
	tc_types::errors_t) : @boolean_t::type {
	var ret = ptd_system::is_accepted_info(from, as_type, ref modules, ref errors);
	return ov::is(ret, 'ok');
}
def ptd_system::is_accepted_info(from : @tc_types::type, as_type : @tct::meta_type, ref modules : @tc_types::modules_t, ref errors : @
	tc_types::errors_t) : @tc_types::check_info {
	from = ptd_system::can_delete(from, ref modules, ref errors);
	return :ok if ov::is(from->type, 'tct_im');
	if(ov::is(as_type, 'tct_rec')){
		return :ok if hash::size(ov::as(as_type, 'tct_rec')) == 0 && ov::is(from->type, 'tct_rec');
	} elsif(ov::is(as_type, 'tct_var')){
		return :ok if hash::size(ov::as(as_type, 'tct_var')) == 0 && ov::is(from->type, 'tct_var');
	}
...
def add_ref_name(ref from : @tct::meta_type, ref hash, ref arr, level : ptd::sim(), ref modules : @
	tc_types::modules_t, ref errors : @tc_types::errors_t) : ptd::sim() {
...
def is_cycle_ref(ref a : @tct::meta_type, ref b : @tct::meta_type, ref ref_inf : @tc_types::ref_t, is_cross : @boolean_t::type,
		deleted: @boolean_t::type, ref modules : @tc_types::modules_t, ref errors : @tc_types::errors_t) : @boolean_t::type {
	if (ov::is(a, 'tct_ref') && ov::is(b, 'tct_ref')) {
		return true if ov::as(a, 'tct_ref') eq ov::as(b, 'tct_ref');
		return true if is_cross && ov::is(check_assignment_info(a, b, ref_inf, :speculation,  ref modules, ref errors), 'ok');
	}
...
def cross_type(a : @tct::meta_type, b : @tct::meta_type, ref_inf : @tc_types::ref_t, ref modules : @tc_types::modules_t, ref errors : 
	@tc_types::errors_t) : @tct::meta_type {
	return :tct_im if(ov::is(b, 'tct_im')||ov::is(a, 'tct_im'));
	if (is_cycle_ref(ref a, ref b, ref ref_inf, true, false, ref modules, ref errors)) {
		return a;
	}
	if (ref_inf->level == 200) {
		add_error(ref errors, 'can''t assignment two that diffrent type to one var');
		return :tct_im;
	}
	return a if(ov::is(b, 'tct_empty'));
	match (a) case :tct_empty {
		return b;
	} case :tct_im {
		return :tct_im;
	} case :tct_sim {
		return :tct_sim if ov::is(b, 'tct_sim');
	} case :tct_ref(var ref_name) {
		die;
	} case :tct_void {
		die;
		...
		if (ov::is(b, 'tct_arr')) {
			return tct::tct_arr(cross_type(arr, b as :tct_arr, ref_inf, ref modules, ref errors));
		}
				...
				if (hash::has_key(ret, field)) {
					...
					match (t2) case :with_param(var typ) {
						...
						match (type) case :with_param(var typ) {
							...
							hash::set_value(ref fin, field, tct::tct_none());
						}
					}
				} else {
					match (type) case :with_param(var typ) {
		...
		if (ov::is(b, 'tct_rec')) {
			...
			if(err){
				...
				return tct::tct_hash(cross_type(reta, retb, ref_inf, ref modules, ref errors));
			} else {
				var ret = {};
				forh var field, var type (reca) {
					hash::set_value(ref ret, field, cross_type(type, hash::get_value(recb, field), ref_inf, ref modules, ref errors));
				}
				return tct::tct_rec(ret);
			}
		}
		if (ov::is(b, 'tct_hash')) {
			var sum = ptd_system::rec_to_hash(a, ref_inf, ref modules, ref errors);
			return tct::tct_hash(ptd_system::cross_type(b as :tct_hash, sum, ref modules, ref errors));
		}
	} case :tct_hash(var hash) {
		if (ov::is(b, 'tct_hash')) {
			return tct::tct_hash(cross_type(hash, b as :tct_hash, ref_inf, ref modules, ref errors));
		}
		if (ov::is(b, 'tct_rec')) {
			var sum = ptd_system::rec_to_hash(b, ref_inf, ref modules, ref errors);
			return tct::tct_hash(ptd_system::cross_type(hash, sum, ref modules, ref errors));
		}
	}
	return :tct_im;
}
def ptd_system::rec_to_hash(a : @tct::meta_type, ref_inf : @tc_types::ref_t, ref modules : @tc_types::modules_t, ref errors : @tc_types::errors_t) : @tct::meta_type {
	var ret = tct::tct_empty();
...
def ptd_system::cast_type(to : @tct::meta_type, from : @tc_types::type, ref modules : @tc_types::modules_t, ref errors : @tc_types::errors_t) : @tc_types::check_info {
...
def ptd_system::check_assignment(to : @tct::meta_type, from : @tc_types::type, ref modules : @
	tc_types::modules_t, ref errors : @tc_types::errors_t) : @tc_types::check_info {
...
def check_assignment_info(to : @tct::meta_type, from : @tct::meta_type, ref_inf : @tc_types::ref_t, type_src : @tc_types::value_src, ref modules : @
	tc_types::modules_t, ref errors : @tc_types::errors_t) : @tc_types::check_info {
	return :ok if ov::is(from, 'tct_empty');
	return mk_err(to,from) if ov::is(from, 'tct_void');
	if(ov::is(to, 'tct_im')){
		walk_on_type(from, :delete, ref_inf->from, ref modules, ref errors) if ref_inf->check && ptd_system::is_known(type_src);
		return :ok;
	}
	...
	match (to) case :tct_im {
		...
		return mk_err(to,from) unless ov::is(from, 'tct_arr');
		var sub_check = check_assignment_info(arr_type, from as :tct_arr, ref_inf, type_src, ref modules, ref errors);
		match(sub_check) case :ok{
			return :ok;
		} case :err(var info){
		...
		if (ov::is(from, 'tct_rec') && !ov::is(type_src, 'known')) {
			forh var name, var record (from as :tct_rec) {
				var sub_check = check_assignment_info(hash_type, record, ref_inf, type_src, ref modules, ref errors);
				match(sub_check) case :ok{
				} case :err(var info){
		...
		return mk_err(to,from) unless ov::is(from, 'tct_hash');
		var sub_check = check_assignment_info(hash_type, from as :tct_hash, ref_inf, type_src, ref modules, ref errors);
		match(sub_check) case :ok{
			return :ok;
		} case :err(var info){
		...
		if(ref_inf->cast && from is :tct_hash){
			...
			forh var name, var record (records) {
				var sub_check = check_assignment_info(record, left, ref_inf, type_src, ref modules, ref errors);
				match(sub_check) case :ok{
				} case :err(var info){
					array::push(ref info->stack, :ptd_rec(name));
					return :err(info);
		...
		return mk_err(to,from) unless ov::is(from, 'tct_rec');
		...
		forh var name, var record (records) {
			...
			var sub_check = check_assignment_info(record, cand_record, ref_inf, type_src, ref modules, ref errors);
			match(sub_check) case :ok{
			} case :err(var info){
				array::push(ref info->stack, :ptd_rec(name));
				return :err(info);
		...
		return :ok if ov::is(from, 'tct_sim');
		return mk_err(to,from);
	} case :tct_var(var vars) {
		return mk_err(to,from) unless ov::is(from, 'tct_var');
		var from_var = from as :tct_var;
		#return mk_err(to,from) if hash::size(from_var) != hash::size(vars) && ptd_system::is_known(type_src);
		forh var name, var from_type (from_var) {
			...
			match (from_type) case :no_param{
				...
				match (to_type) case :no_param{
					...
					var sub_check = check_assignment_info(t_t, f_t, ref_inf, type_src, ref modules, ref errors);
					match(sub_check) case :ok{
					} case :err(var info){
...
def add_delete(type_name : ptd::sim(), ref modules : @tc_types::modules_t, ref errors : @
	tc_types::errors_t) : ptd::void() {
	array::push(ref modules->env->deref->delete, {line=>errors->current_line, module=>modules->env->current_module, type_name=>type_name});
}
def add_create(type_name : ptd::sim(), ref modules : @tc_types::modules_t, ref errors : @
	tc_types::errors_t) : ptd::void() {
	array::push(ref modules->env->deref->create, {line=>errors->current_line, module=>modules->env->current_module, type_name=>type_name});
}
def ptd_system::can_delete(from : @tc_types::type, ref modules : @tc_types::modules_t, ref errors : @
...
def ptd_system::can_create(from : @tc_types::type, ref modules : @tc_types::modules_t, ref errors : @
	tc_types::errors_t) : @tc_types::type {
	while (ov::is(from->type, 'tct_ref')) {
		var type_name = from->type as :tct_ref;
...
def walk_on_type(type : @tct::meta_type, operation : ptd::var({create=>ptd::none(), delete=>ptd::none()}), ref_inf : ptd::hash(ptd::arr(ptd::sim())), ref modules : 
		@tc_types::modules_t, ref errors : @tc_types::errors_t) : ptd::void(){
	var refs = {};
...
def get_all_ref_in_type(type : @tct::meta_type, ref refs : ptd::hash(ptd::sim()), ref modules : 
		@tc_types::modules_t, ref errors : @tc_types::errors_t) : ptd::void() {
	var refs2 = {};
...
def get_function_def(type_name : ptd::sim(), ref modules : @tc_types::modules_t, ref errors : @
	tc_types::errors_t) : ptd::arr(@tc_types::def_fun_t) {
	var module : ptd::sim();
